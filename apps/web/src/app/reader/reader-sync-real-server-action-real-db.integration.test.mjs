...
function buildTestPrefix() {
  return `lap_a327_reader_sync_wrapper_${Date.now()}_${randomUUID().replace(/-/g, "")}`;
}
...
function makeTrustedAuthSessionInput(ids, prefix) {
  return {
    previewOnly: true,
...
function makeTrustedServerContext(ids) {
  return {
    hasAuthenticatedUser: true,
    serverUserId: ids.userId,
    canAccessBook: true,
...
function makeLocalProgress(ids, progressRatio) {
  return {
    bookId: ids.bookId,
...
test(
  "real Reader sync wrapper integration runs only on an explicit local/test DB opt-in",
  {
  ...
  async function () {
    ...
    const authSessionAdapter = createMockReaderSyncAuthSessionAdapterForTest(
      makeTrustedAuthSessionInput(ids, prefix),
    );
    const trustedServerContext = makeTrustedServerContext(ids);
    ...
    try {
      ...
      await prisma.user.create({
        data: {
          ...
          name: "A327 Reader Sync Test User",
        },
      });
      ...
      await prisma.book.create({
        data: {
          ...
          title: "A327 Reader Sync Test Book",
          ownerId: ids.userId,
        },
      });
      ...
      await prisma.bookChapter.create({
        data: {
          id: ids.chapterId,
          bookId: ids.bookId,
          title: "A327 Reader Sync Test Chapter",
          level: 1,
          orderIndex: 1,
        },
      });
      ...
      const firstResult = await executeReaderSyncRealServerActionForTest(
        {
          localProgress: makeLocalProgress(ids, incomingProgressRatio),
        },
        {
          allowTestRealDbExecution: true,
          authSessionAdapter,
          trustedServerContext,
          repositoryAdapter: harness.webAdapter,
          dbIntegrationGuardPreview: guardPreview,
        },
      );

      assert.equal(firstResult.testOnly, true);
      assert.equal(firstResult.devOnly, true);
      assert.equal(firstResult.realDbIntegrationTest, true);
      assert.equal(firstResult.executionAttempted, true);
      assert.equal(firstResult.executionSucceeded, true);
      assert.equal(firstResult.executionAllowed, true);
      assert.equal(firstResult.executionMode, "test-dev-only-real-db");
      assert.equal(firstResult.source, "test-dev-only");
      assert.equal(firstResult.serverContextStub.source, "trusted-server-context");
      assert.equal(firstResult.serverContextStub.serverUserId, ids.userId);
      assert.equal(firstResult.authSessionPreview.source, "test-only-mock");
      assert.equal(firstResult.authSessionPreview.snapshot.serverUserId, ids.userId);
      assert.equal(firstResult.authSessionPreview.snapshot.authSessionVerified, true);
      assert.equal(firstResult.executionGuardPreview.allowed, true);
      assert.equal(firstResult.executionGuardPreview.dbIntegrationGuardPreview.canRunDbIntegrationTest, true);
      assert.equal(firstResult.corePreview.status, "test_only_fake_preview");
      assert.equal(firstResult.corePreview.testOnlyExecutionPreview.attempted, true);
      assert.equal(firstResult.corePreview.testOnlyExecutionPreview.executed, true);
      assert.equal(firstResult.corePreview.testOnlyExecutionPreview.success, true);
      assert.equal(firstResult.corePreview.serviceResultPreview.status, "ready_preview");
      assert.equal(firstResult.corePreview.serviceResultPreview.persistentAdapterPreview.executed, true);
      assert.equal(firstResult.corePreview.serviceResultPreview.persistentAdapterPreview.success, true);
      assert.equal(firstResult.corePreview.serviceResultPreview.persistentAdapterPreview.callsRepository, true);
      ...
      const regressionResult = await executeReaderSyncRealServerActionForTest(
        {
          localProgress: makeLocalProgress(ids, regressionProgressRatio),
        },
        {
          allowTestRealDbExecution: true,
          authSessionAdapter,
      ...
      assert.equal(regressionResult.testOnly, true);
      assert.equal(regressionResult.devOnly, true);
      assert.equal(regressionResult.realDbIntegrationTest, true);
      assert.equal(regressionResult.executionAttempted, true);
      assert.equal(regressionResult.executionSucceeded, false);
      assert.equal(regressionResult.executionAllowed, true);
      assert.equal(regressionResult.executionMode, "test-dev-only-real-db");
      assert.equal(regressionResult.source, "test-dev-only");
      assert.equal(regressionResult.corePreview.status, "test_only_fake_preview");
      assert.equal(regressionResult.corePreview.testOnlyExecutionPreview.attempted, true);
      assert.equal(regressionResult.corePreview.testOnlyExecutionPreview.executed, false);
      assert.equal(regressionResult.corePreview.testOnlyExecutionPreview.success, false);
      assert.equal(
        regressionResult.corePreview.serviceResultPreview.persistentAdapterPreview.status,
        "conflict",
      );
