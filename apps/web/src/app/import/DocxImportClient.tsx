...
import { docxImportServerAction } from "../../lib/docx-import-server-action";
import type {
  DocxImportActionResult,
  DocxImportChapterPreview,
} from "../../lib/docx-import-server-action";
import { DOCX_MAX_FILE_SIZE } from "../../lib/docx-import-parser";

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

interface DocxImportClientState {
  status: "idle" | "uploading" | "success" | "error";
  result: DocxImportActionResult | null;
...
export function DocxImportClient({
  ...
  const [state, setState] = useState<DocxImportClientState>({
    status: "idle",
    result: null,
    errorMessage: null,
  });
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        const result = await docxImportServerAction(formData);
        if (result.success) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {guardEnabled ? "DOCX 导入：开发预览（已启用）" : "DOCX 导入：已阻止"}
        </strong>
        <p
          style={{
            margin: "4px 0 0",
      ...
      >
        ...
        >
          <span>仅接受 .docx 文件</span>
          <span>大小上限：{DOCX_MAX_FILE_SIZE / (1024 * 1024)} MB</span>
          <span>仅纯文本提取，不保留样式/图片/批注</span>
          <span>不调用 LLM</span>
        </div>
      </form>
...
function DocxImportSuccessPanel({
  result,
  onReset,
}: {
  result: DocxImportActionResult & { success: true };
  onReset: () => void;
}) {
  return (
    <div
    ...
    >
      <strong style={{ color: "#166534" }}>DOCX 文本提取成功</strong>
      ...
      >
        已提取纯文本，可进入文本导入预览确认并保存。不保留样式/图片/批注，不调用 LLM。
      </p>
...
function ChapterPreviewList({
  ...
  chapters: DocxImportChapterPreview[];
}) {
  return (
    <div
      style={{
