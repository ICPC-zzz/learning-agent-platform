...
test("A523 email provider supports Resend production env aliases", () => {
  assert.match(configSource, /RESEND_API_KEY/);
  assert.match(configSource, /LAP_EMAIL_API_KEY/);
  assert.match(configSource, /RESEND_FROM_EMAIL/);
  assert.match(configSource, /EMAIL_FROM/);
  assert.match(configSource, /LAP_EMAIL_FROM/);
...
test("A523 complete provider config enables real email path without dev console fallback", () => {
  assert.match(sendSource, /getEmailRuntimeConfig\(\)\.realSendAllowed/);
...
test("A523 env example lists placeholders only for email provider", () => {
  ...
  assert.match(envExample, /RESEND_API_KEY="re_your_key"/);
});
